# Writing Construction Guide — Visual-First Prose for Frame Decomposition

This guide governs ALL prose output from the Creative Coordinator. Every spawned agent writing narrative content MUST follow these rules. Your prose is the first establishment of visual flow in the pipeline — understanding how it gets consumed downstream is not optional, it is the foundation of how you write.

---

## Core Principle: The Six Elements of Visual Flow
...
**The cycle:** Your prose should move through these elements fluidly. A scene might open with establishment, introduce a character through motion, land on dialogue with physical business, capture a reaction, escalate through action, and resolve with weight. Then the next beat begins. The specific order varies — but stacking (3 paragraphs of pure establishment, or 4 dialogue blocks in a row) breaks the visual rhythm.
...
| **F04** | Dialogue | Close-up speaking shot (single speaker) |
| **F05** | Dialogue | Over-shoulder dialogue exchange |
| **F06** | Dialogue | Wide dialogue (characters + environment visible) |
| **F07** | Environment | Establishing shot (full location) |
| **F08** | Environment | Detail/atmosphere shot (object, texture, light) |
...
| **F17** | Transition | Narrative transition (scene bridge, fade concept) |
| **F18** | Cinematic | Cinematic emphasis (slow push, rack focus, reveal) |

### The Two Frame Categories and How They Alternate

| Visual Frame (WHO/WHERE) | Event Frame (WHAT HAPPENS) |
|---|---|
...
Every dialogue exchange produces a minimum of TWO frames:
...
3. **Action & staging** — physical movement, body positions, changes
4. **Characters** — who is present, expression, pose

Your prose should mirror this order. The environment is the canvas. Characters are placed INTO it. When you write "Mei crosses to the window" you've given priority 3-4 but skipped 1-2. When you write "Dusk light through lattice, striped shadow on floorboards. Mei crosses to the window." you've given all four layers.
...
**Good — 3 frames:**
> Mei rises from the stool. The silk pools off her lap and settles.
>
> She crosses to the window. Dusk light catches the lattice, throws striped shadow across her face.
>
...
Use keyword-cluster descriptions for efficiency: `Amber light through lattice. Powder motes. Lacquered screens, ivory combs, silk robes on a carved stand.` This gives the image generator maximum material in minimum tokens. Save literary prose for character moments that earn it.

### 3. Characters Act While They Talk
...
```
                    MADAME CHOU
          (projecting, formal)
    Gentlemen. The jewel of Lu Xian.
    She plays Go at provincial level.
    Her conversation has made old men
    feel young, and young men feel wise.
```

**Good — 4-5 frames, camera in motion:**
```
Chou extends her hand toward Mei — palm up, presenting.

                    MADAME CHOU
          (projecting, walking the crescent — WIDE F06)
    Gentlemen. The jewel of Lu Xian.

Her fingers brush Mei's shoulder as she turns to the wealthiest tables. Zhao's tea bowl pauses at his lips.

                    MADAME CHOU
          (turning to face Zhao — MS, over-shoulder)
    Her conversation has made old men
    feel young, and young men feel wise.
...
- Handling objects (pouring tea, gripping a railing, turning a letter)
- Gesturing toward someone or something
- Making/breaking eye contact with a specific person
- Adjusting clothing, hair, posture
- Physical contact with another character
- Turning away, turning back
- Sitting down, standing up, leaning

**What does NOT count (not frameable):**
- "She pauses" — not visual
- "His voice changes" — audio only
- "Silence fills the room" — nothing to photograph

### 4. Shot-Aware Parentheticals
...
| WIDE | Wide/full shot (full body + environment) |
| OTS | Over-the-shoulder |
| 2-SHOT | Two characters in frame |
| POV | Point-of-view from this character |
| HIGH | High angle looking down |
| LOW | Low angle looking up |

Camera movements: `push-in`, `pull-back`, `pan`, `track`, `static`, `handheld`

**Examples:**
```
          (whispered, desperate — ECU, static)
          (commanding, sweeping the room — WIDE, slow pan)
          (bitter, not looking at her — OTS from her POV, static)
...
When a character speaks more than ~30 words, break the speech into segments separated by physical business — the speaker's own body, a listener's reaction, or an environment shift. The dialogue continues; the camera has somewhere to go.

**Pattern for long speeches:**
```
[Physical setup — speaker begins an action]

    DIALOGUE BLOCK 1

[Visual beat — listener reacts, or speaker's body shifts]
...
When a character is alone (reading a letter, making a decision, waiting), they still ACT. The body externalizes the interior:

- They move through space — rising, crossing rooms, changing positions
- They interact with objects — holding, turning, pressing to chest, setting down
- They make physical choices that mirror emotional ones — opening a door, closing a shutter, gripping a railing then releasing it
- Light and environment respond — a candle flickers, rain starts, shadows shift

**Every internal beat needs an external expression.** "She decided" is unframeable. "She closes her fingers around the pouch. Her jaw sets." produces two frames (F11 object interaction + F01 character portrait).
...
Static backgrounds produce dead frames. In every environment description, include at least one element of motion or life:

- Wind stirring fabric, leaves, dust, petals
- Distant human activity — vendors, passersby, servants
- Animals — a bird crossing a gap, a dog trotting past, insects in lantern light
- Light in motion — flickering candles, shifting sun, moving shadows
- Sound made visible — rain on surfaces, steam rising, breath in cold air

### 9. Token Efficiency

Write dense, not long. Every sentence should either:
1. Establish a frameable visual moment, OR
2. Advance plot/character, OR
3. Provide a dialogue delivery

If a sentence does none of these, cut it. A 50-word paragraph that produces one clear frame is better than a 150-word paragraph that produces one muddy one. Atmosphere earns its place only when it creates a distinct frame.
...
- [ ] No more than 4 consecutive action beats without a breathing frame (F01 or F07)
- [ ] Parentheticals include both performance direction AND shot hint
...
- [ ] Every sentence earns its place (frameable moment, plot advancement, or dialogue delivery)
